...
Ordered_States_t get_ordered_children(State_t state, unsigned long long int hash, Zobrist_Table &zhasher) {
  ...
  for (int my_index = my_player_index[state[21]], end = my_index + 10; my_index < end; ++my_index) {
    if (state[my_index] != 0u) {  // If the piece hasn't been captured
      ...
      start_idx = TO_IDX[to_move_location];
      ...
      for (unsigned int pos : type_to_attack[my_index][to_move_location]) {
        if (pos & opponent_locs & shadows) {
          ...
          state_value value = {
              ...
              zhasher.update_hash(hash, piece_type_zobrist_index[my_index], start_idx, piece_type_zobrist_index[target_index], target_index)
          };
      ...
      for (unsigned int pos : type_to_move[my_index][to_move_location]) {
        if (pos & empty_locs & shadows) {
          ...
          state_value value = {
              ...
              false,  // If not an attack, can't be a win
              // Target type is always empty cell
              zhasher.update_hash(hash, piece_type_zobrist_index[my_index], start_idx, 12, TO_IDX[pos])
          };
          result.emplace(value);
        }
      }
    ...
    if (state[my_index] != 0u) {  // If the piece hasn't been captured
      ...
      for (unsigned int pos : type_to_attack[my_index][to_move_location]) {
        if (pos & opponent_locs & shadows) {
          int target_index = 0;
          for (; target_index < 20 && pos != state[target_index]; ++target_index);
          result.push_back(make_attack(state, my_index, target_index));
        }
      }
      ...
      for (unsigned int pos : type_to_move[my_index][to_move_location]) {
        if (pos & empty_locs & shadows) {
          result.push_back(make_move(state, my_index, pos));
        }
      }
    }
  }
...
int negamax(State_t state, int depth, int alpha, int beta, Zobrist_Table & zhasher) {
  if ((depth == 0) || (state[0] == 0) || (state[19] == 0)) {
    return material_evaluation(state);
  }

  std::vector<State_t> child_states = get_children(state);
  int v;
  int best_value = -50600;
  for (State_t child_state : child_states) {
    v = -negamax(child_state, depth - 1, -beta, -alpha, zhasher);
